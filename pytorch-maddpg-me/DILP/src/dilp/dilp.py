...
class DILP():
    ...
    def show_definition(self):
        for predicate in self.rule_weights:
            ...
            weightsFlat = tf.keras.backend.flatten(weights) #weights.flattened()
            arg = np.argsort(weightsFlat)
            cl_set = []
            for a in arg[-5:]:
                pos = np.unravel_index(
                    a, weights.shape)
                print("weights>>>> ",weights[pos[0],pos[1]])
                print(clauses[0][pos[0]])
                print(clauses[1][pos[1]])
                cl_set.append(clauses[0][pos[0]])
            ...
            '''
            print('----------------------------')
        return cl_set#clauses[0][pos[0]],clauses[1][pos[1]]
    ...
    def train(self, steps=501, name='test'):
        ...
        for i in range(steps):
            ...
            if i % 50 == 0:
                cl_set = self.show_definition()
                # if name:
                # checkpoint.save(checkpoint_prefix)
                # pd.Series(np.array(losses)).to_csv(name + ".csv")
            print("-" * 20 + "\n")
        return losses,cl_set
