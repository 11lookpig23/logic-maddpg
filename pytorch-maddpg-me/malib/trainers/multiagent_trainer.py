...
class MATrainer:
    ...
    def run(self):
        ...
        #np.save("train_container.npy", self.sampler.container)
        #np.save("train_loss.npy", agents_losses)
        with open('train_container_ag2lift.txt', 'wt') as f:
            print( self.sampler.container, file=f)
        with open('train_loss_ag2lift.txt', 'wt') as f:
            print( self.losses, file=f)
